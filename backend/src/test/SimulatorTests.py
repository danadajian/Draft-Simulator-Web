...
pos_dict = {'Saquon Barkley': 'RB', 'Christian McCaffrey': 'RB', 'Alvin Kamara': 'RB', 'Ezekiel Elliott': 'RB',
            ...
            'Titans D/ST': 'DST', 'Greg Zuerlein': 'K', 'Justin Tucker': 'K', 'Wil Lutz': 'K', 'Harrison Butker': 'K',
            ...
            'Matt Prater': 'K', 'Jake Elliott': 'K', 'Noah Fant': 'TE', 'Justice Hill': 'RB', 'Justin Jackson': 'RB',
            ...
            'Derek Carr': 'QB', 'DeVante Parker': 'WR', 'Zay Jones': 'WR', 'Chris Conley': 'WR', 'Cole Beasley': 'WR',
            'Quincy Enunwa': 'WR', 'Willie Snead IV': 'WR', 'Sam Darnold': 'QB', 'Jimmy Garoppolo': 'QB',
            'Eagles D/ST': 'DST', 'Browns D/ST': 'DST', 'Mason Crosby': 'K', 'Ryan Succop': 'K', 'Josh Doctson': 'WR',
...
class SimulateDraftTests(unittest.TestCase):
    ...
    def test_pick_players_first_round(self):
        ...
        rounds = 1
        self.assertTrue(pick_players(players, teams, pos_dict, order, rounds) == ['Saquon Barkley'])
    ...
    def test_pick_players_later_round(self):
        ...
        rounds = 2
        self.assertTrue(
            pick_players(players, teams, pos_dict, order, rounds) == ['Saquon Barkley', 'Patrick Mahomes'])
    ...
    def test_pick_players_later_round_random(self):
        ...
        order = set_draft_order(teams, pick)
        rounds = 2
        self.assertTrue(pick_players(players, teams, pos_dict, order, rounds)[0] == 'Saquon Barkley')
        self.assertTrue(len(pick_players(players, teams, pos_dict, order, rounds)) == rounds)
    ...
    def test_pick_players_missing_round(self):
        ...
        order = set_draft_order(teams, pick)
        rounds = 16
        first_round_pick = pick_players(players, teams, pos_dict, order, rounds)[0]
        fourth_round_pick = pick_players(players, teams, pos_dict, order, rounds)[3]
    ...
    def test_drafted_teams_has_correct_form(self):
        ...
        rounds = 5
        sims = 10
        self.assertTrue(len(simulate_draft(players, pos_dict, teams, pick, rounds, sims)) == sims)
        self.assertTrue(
            all(len(team) == rounds for team in simulate_draft(players, pos_dict, teams, pick, rounds, sims)))
    ...
    def test_get_top_pick_one_sim(self):
        ...
        rounds = 1
        sims = 1
        self.assertTrue(simulate_draft(players, pos_dict, teams, pick, rounds, sims) == [['Saquon Barkley']])
    ...
    def test_get_top_pick_many_sims(self):
        ...
        sims = 10
        self.assertTrue(
            all(team == ['Saquon Barkley'] for team in simulate_draft(players, pos_dict, teams, pick, rounds, sims)))
    ...
    def test_dont_get_top_pick_many_sims(self):
        ...
        self.assertTrue(not all(
            team == ['Saquon Barkley'] for team in simulate_draft(players, pos_dict, teams, pick, rounds, sims)))
    ...
    def test_full_draft_position_counts_valid_one_sim(self):
        ...
        rounds = 16
        sims = 1
        team = simulate_draft(players, pos_dict, teams, pick, rounds, sims)[0]
        pos_counts = {'QB': 2, 'RB': 5, 'WR': 5, 'TE': 2, 'DST': 1, 'K': 1}
        self.assertTrue(all(position_count(team, pos_dict, pos) == pos_counts.get(pos) for pos in pos_counts.keys()))
    ...
    def test_full_draft_position_counts_valid_many_sims(self):
        ...
        rounds = 16
        sims = 10
        drafted_teams = simulate_draft(players, pos_dict, teams, pick, rounds, sims)
        pos_counts = {'QB': 2, 'RB': 5, 'WR': 5, 'TE': 2, 'DST': 1, 'K': 1}
        self.assertTrue(all(position_count(team, pos_dict, pos) == pos_counts.get(pos) for pos in pos_counts.keys())
    ...
    def test_get_expected_team(self):
        ...
        self.assertEqual(['Saquon Barkley', 'Evan Engram', 'Travis Kelce', 'Golden Tate'],
                         get_expected_team(draft_freqs, pos_dict, round_dict, 4))
    ...
    def test_get_expected_team_invalid_choices(self):
        ...
        self.assertEqual(['Drew Brees', 'Patrick Mahomes', 'Zach Ertz', 'Saquon Barkley'],
                         get_expected_team(draft_freqs, pos_dict, round_dict, 5))
    ...
    def test_order_expected_team_and_assign_positions(self):
        ...
        self.assertEqual({'Aaron Rodgers': 'QB', 'Saquon Barkley': 'RB', 'James Conner': 'RB', 'Odell Beckham Jr': 'WR',
                          ...
                          'Jamison Crowder': 'BE'}, order_expected_team_and_assign_positions(team, pos_dict))
    ...
    def test_order_short_team_bench_only(self):
        ...
        self.assertEqual({'Aaron Rodgers': 'QB', 'Eli Manning': 'BE', 'Carson Wentz': 'BE', 'Saquon Barkley': 'RB'},
                         order_expected_team_and_assign_positions(team, pos_dict))
    ...
    def test_jsonify_stuff(self):
        ...
        self.assertEqual([{"Name": "Aaron Rodgers", "Position": "QB", "Team": "GB", "Round": 1, "Frequency": "0%"},
                         ...
                         jsonify(players, pos_dict, team_dict, round_dict, freq_dict))
