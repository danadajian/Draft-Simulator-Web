...
class OptimizerTests(unittest.TestCase):
    def test_best_lineup(self):
        matrix = ['QB', 'RB']
        proj_pts = {'Eli': 20, 'Gurley': 25, 'Saquon': 69}
        pos_dict = {'Eli': 'QB', 'Gurley': 'RB', 'Saquon': 'RB'}
        self.assertEqual([['Eli', 'Saquon'], [['Eli'], ['Saquon', 'Gurley']], {'Saquon': 69, 'Gurley': 25, 'Eli': 20}],
                         best_lineup(matrix, proj_pts, pos_dict))

    def test_higher_proj_pts(self):
        position_matrix = {'QB': [1], 'RB': [1]}
        proj_pts = {'Eli': 20, 'Gurley': 25, 'Saquon': 69}
        pos_dict = {'Eli': 'QB', 'Gurley': 'RB', 'Saquon': 'RB'}
        lineup = best_lineup(position_matrix, proj_pts, pos_dict)
        best, pools, proj_dict = lineup[0], lineup[1], lineup[2]
        salary_dict = {'Eli': 5000, 'Gurley': 6500, 'Saquon': 6500}
        salary_cap = 12000
        self.assertEqual(['Eli', 'Saquon'], optimize(best, pools, proj_dict, salary_dict, salary_cap))
    ...
    def test_salary_cap(self):
        ...
        salary_cap = 12000
        self.assertEqual(['Eli', 'Gurley'], optimize(best, pools, proj_dict, salary_dict, salary_cap))
    ...
    def test_mult_of_eac_pos(self):
        ...
        salary_cap = 12000
        result = set(optimize(best, pools, proj_dict, salary_dict, salary_cap))
    ...
    def test_salary_cap_too_low(self):
        ...
        salary_cap = 100
        result = optimize(best, pools, proj_dict, salary_dict, salary_cap)
        self.assertEqual('Not enough players to sufficiently downgrade.', result)

    def test_not_enough_players(self):
        position_matrix = ['QB', 'RB', 'RB', 'WR', 'WR', 'RB WR']
        proj_pts = {'QB1': 5, 'RB1': 2, 'RB3': 1, 'RB4': 6, 'WR1': 3, 'WR2': 5}
    ...
    def test_ignore_player(self):
        ...
        new_pools = ignore_players('QB3', black_list, pools)
        self.assertEqual([['QB2', 'QB1'], ['RB2', 'RB3'], ['WR3']], new_pools)

    def test_ignore_no_players(self):
        black_list = []
        pools = [['QB2', 'QB3', 'QB1'], ['RB2', 'RB3', 'RB1'], ['WR1', 'WR3']]
        new_pools = ignore_players('', black_list, pools)
